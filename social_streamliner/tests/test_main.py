...
class TestMainApp(unittest.TestCase):
    ...
    def test_webhook_success(self, mock_save, mock_send):
        ...
        # Ora includiamo anche gameName, che è obbligatorio
        webhook_data = {
    ...
    def test_webhook_bad_request_missing_fields(self):
        """Testa un errore 400 se mancano 'videoUrl' o 'gameName'."""
        # Manca gameName
        response = self.client.post('/webhook',
        ...
        # Manca videoUrl
        response = self.client.post('/webhook',
                                    data=json.dumps({'gameName': 'TestGame'}),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 400)
    ...
    def test_callback_approve_success(self, mock_get_clip_data, mock_gen_content, mock_add_cal, mock_send_confirm):
        """Testa il flusso di approvazione con successo."""
        # Simula il ritorno di un dizionario completo da parte della funzione
        mock_clip_data = {
            "video_url": "http://retrieved.url",
            "game_name": "RetrievedGame",
            "clip_details": "Some details from sheet"
        }
        mock_get_clip_data.return_value = mock_clip_data
        mock_gen_content.return_value = {"title": "AI Title", "description": "AI Desc", "hashtags": ["#ai"]}
        mock_add_cal.return_value = True
        ...
        self.assertEqual(response.status_code, 200)
        mock_get_clip_data.assert_called_once_with("clip123")
        # Verifica che generate_content sia chiamato con i dati corretti
        mock_gen_content.assert_called_once_with("RetrievedGame", "Some details from sheet")
        mock_add_cal.assert_called_once()
